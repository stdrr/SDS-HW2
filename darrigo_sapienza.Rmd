...
The `likelihood` function is a routine used into the `handmade.em` and calculates, for each observation, the corresponding likelihood probability.
...
handmade.em <- function(y, p, mu, sigma, n.iter, plot.flag = T, tol = 0.0001)
{
  ...
  for (iter in 1:n.iter) { # start i-th iteration
    ...
    if(abs(deviance - deviance.pred) <= tol) {  
      # if the change of the likelihood of each sample is no more than the tolerance threshold, then EM converged 
      print(paste("EM converged after", iter, "iterations"))
      break # exit from the loop
    }
...
initialize.parameters <- function(y, k) {
  ...
  p <- c(0.5, rep(0.1, k-1)) # equally weight the k components
...
Now, We want to see which metric has predicted really well the true kmax for the bart distribution considering kmax_true = 6:
...
We displayed, the through the iterations of six gaussian parameters the changment of the parameters at each iteration:
...
plot.params(XX, 100, kmax=6)
```

We want to make different types of inferences, like seeing also the confidence interval of our n samples defined as n1=40 and n2=1000:

```{r}
# Establish the confidence interval
confidence.interval <- function(n) {
  x_hat <- mean(n); se_hat <- sd(n)
  return(c(x_hat - 1.96 * se_hat, x_hat + 1.96 * se_hat))
}

n1 <- bart.samples(40)
cf.n1 <- confidence.interval(n1)

n2 <- bart.samples(1000)
cf.n2 <- confidence.interval(n2)
```

Now, I want to say the percentage of points that fall into the region:

```{r}
falling.in.cf <- function(n, cf) {
  return( round( sum(sapply(n, function(x) return((x >= cf[1]) && (x <= cf[2])) ) ) / length(n), 3) * 100 )
}

print(paste("The ", falling.in.cf(n1, cf.n1), "% of the n1 data fall into the region", sep=""))
print(paste("The ", falling.in.cf(n2, cf.n2), "% of the n2 data fall into the region", sep=""))
```
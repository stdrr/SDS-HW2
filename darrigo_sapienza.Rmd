...
```{r}
penalty.model <- function(y, kmax, n.iter, AIC=FALSE, BIC=FALSE) {
  best.j <- c(); len.y <- length(y);
  ...
  for (k in 1:kmax){
    ...
    if (AIC) {
      best.j <- c(best.j, 2 * sum(log(like)) - 2 * (3*k-1)) # calculate the AIC #### DESCRIBE WELL
    } else {
      best.j <- c(best.j, sum(log(like)) - log(len.y)/2 * (3*k-1))
    }  
  }
  
  best.k <- which.max(best.j) # return the index of the k component the maximize the score 
  return(best.k)
}
```
...
wass.score <- function(y.train, y.test, kmax, n.iter) { # routine function useful to apply the Wasserstein formula
  ...
  best.wass <- which.min(wass.sc) # return the index of the k component the minimize the score
  return(best.wass)
}
```
...
do.simulate <- function(n, M, n.iter, kmax) {
  ...
  for (i in 1:M) {
    ...
    best.aic.k <- c(best.aic.k, penalty.model(XX, kmax=kmax, n.iter, AIC=TRUE))
    
    best.bic.k <- c(best.bic.k, penalty.model(XX, kmax=kmax, n.iter, BIC=TRUE))
...
```{r}
# try to simulate with n1 = 25 
do.simulate(n1, M=50, n.iter=20, kmax=8)

# try to simulate with n1 = 2000 
do.simulate(n2, M=50, n.iter=20, kmax=8)
```
...
<!--
PLOT & SUMMARY
  Considering a plot for AIC and BIC where there is k on x axis and score on y axis
  Considering a plot for K-folds where there is k on x axis and score on y axis
  Considering a plot for Splits where there is k on x axis and score on y axis
  
  Considering a scatterplot for wass score and see the correlation & covariance between data and the max score

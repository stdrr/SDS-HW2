...
handmade.em <- function(y, p, mu, sigma, n.iter, plot.flag = T)
{
  ...
  for (iter in 1:n.iter) { # start i-th iteration
    ...
    collapsing <- (sigma <= 0.001)
    sigma[collapsing] <- runif(1, min = 1, max = 2)
    mu[collapsing] <- rnorm(n = 1, mean = mean(y), sd = sd(y))
    
    if(all((likelihood(y, p, mu, sigma) - like) <= 0.0001)) {
      print(paste("1 converged after", iter, "iterations"))
      break
    }
...
We want to make different types of inferences, like seeing also the confidence interval of our n samples defined as n1=150 and n2=2000:

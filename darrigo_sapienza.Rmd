...
AIC <- function(y, kmax, n.iter) {
  ...
  for (k in 1:kmax){
    ...
    aic.j <- c(aic.j, 2 * sum(log(like)) - 2 * (3*k-1)) # calculate the AIC #### DESCRIBE WELL
  }
  
  best.k <- which.max(aic.j) # return the index of the k component the maximize the score 
  return(best.k)
}
```
...
BIC <- function(y, kmax, n.iter) {
  ...
  for (k in 1:kmax){
    ...
    bic.j <- c(bic.j, sum(log(like)) - log(len.y)/2 * (3*k-1)) # calculate the AIC #### DESCRIBE WELL
  }
  
  best.k <- which.max(bic.j) # return the index of the k component the maximize the score 
  return(best.k)
}
```
...
do.simulate(n1, M=10, n.iter=20, kmax=6)

# try to simulate with n1 = 2000 
do.simulate(n2, M=10, n.iter=20, kmax=6)
```
